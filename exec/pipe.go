...
// Pipeline represents commands connected by pipes.
type Pipeline struct {
	cmds []*Cmd
	ctx  *context.Context // if true, the command is a context command
}

// Pipe appends commands to the pipeline.
func (p *Pipeline) Pipe(subcommands ...*Cmd) *Pipeline {
	p.cmds = append(p.cmds, subcommands...)
	return p
}

// PipeCommand parses and appends command strings to the pipeline.
...
// Pipe starts a pipeline from the command and appends additional commands.
...
// PipeCommand starts a pipeline from the command and appends parsed command strings.
...
// Output runs the pipeline and captures output from the final command.
...
// Run runs the pipeline with inherited stdio on the final command.
