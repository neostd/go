...
// WhichOptions configures executable lookup.
type WhichOptions struct {
	// UseCache enables cached lookup results.
	UseCache bool
	// PrependPaths are searched before PATH.
	PrependPaths []string
}

// Which resolves the first matching executable using default options.
func Which(command string) (string, bool) {
	return WhichFirst(command, nil)
}

// WhichFirst resolves the first matching executable using explicit options.
