...
// Mark identifies a location in parsed input.
type Mark struct {
	Line   int
	Column int
}

// Token represents a lexed dotenv token.
type Token struct {
	Type     int
	RawValue []rune
	value    *string
...
// ParseError reports a parse failure with source position information.
...
// Value returns the token's decoded value.
...
// Error returns the parse error text.
func (e *ParseError) Error() string {
	return e.Message + " at line " + strconv.Itoa(e.Line) + ", column " + strconv.Itoa(e.Column)
}

// String returns the parse error text.
func (e *ParseError) String() string {
	return e.Message + " at line " + strconv.Itoa(e.Line) + ", column " + strconv.Itoa(e.Column)
}

// Lex tokenizes dotenv input into a sequence of tokens.
...
// Parse parses dotenv input into an EnvDoc while preserving order and comments.
