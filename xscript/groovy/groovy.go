// Package groovy builds commands for running Groovy files and inline snippets.
package groovy
...
var Extensions = []string{".groovy", ".gvy", ".gy", ".gsh"}
...
func Inline(script string, args ...string) *exec.Cmd {
	allArgs := append(append([]string{}, ScriptArgs...), "-e", script)
	allArgs = append(allArgs, args...)
	return New(allArgs...)
}
...
func Script(script string, args ...string) *exec.Cmd {
	// Treat known file extensions as file execution and everything else as inline code.
